...
Page({
  ...
  onShow: function() {
    ...
    wx.yue.sub("hasToken", function() {
      ...
      app.get_coupons(function(data) {
        if (
          data.length > 0 &&
          wx.Storage.getItem("closeYouhuoquan") === "close"
        ) {
          y.setData({
            hasYouhuiquan: true,
            card: data
  ...
  addToCart(e) {
    ...
    app.isLogin(() => {
      this.getShoppingCartDataLength(data.id, function(thisGoodsLength) {
        ...
        y.setData({
          [data.state === "remai"
            ? `remaiData[${data.index}].showCountControl`
            : `tuijianData[${data.index}].showCountControl`]: true,
          [data.state === "remai"
            ? `remaiData[${data.index}].current_num`
            : `tuijianData[${data.index}].current_num`]: thisGoodsLength + 1
        });
  ...
  numAdd(e) {
    ...
    this.setData({
      [data.state === "remai"
        ? `remaiData[${data.index}].current_num`
        : `tuijianData[${data.index}].current_num`]: num + 1
    });
