...
Page({
  ...
  onShow: function() {
    ...
    if (
      myshippingAddressLength === "undefined" ||
      myshippingAddressLength === ""
    ) {
  ...
  touchE: function(e) {
    ...
    if (e.changedTouches.length == 1) {
      ...
      var txtStyle =
        disX > delBtnWidth / 2 ? "left:-" + delBtnWidth + "rpx" : "left:0px";
      var index = e.currentTarget.dataset.index;
      var list = that.data.cartList;
      list[index].txtStyle = txtStyle;
      that.setData({
  ...
  delItem(e) {
    ...
    var y = this;
    util
      .promiseRequest(api.delete_cart, {
        product_id: id
      })
      .then(res => {
  ...
  checkGoods(e) {
    ...
    this.setData({
      ...
      [`cartList[${index}].haKuCun`]:
        Number(selectData.kucun) >= Number(selectData.num) ? true : false
    });
  ...
  minusNum(e) {
    ...
    let newNum =
      cartList[`${index}`].num > 1 ? cartList[`${index}`].num - 1 : 1;
    // 库存数量
    let kucun = Number(cartList[`${index}`].kucun);
  ...
  toConfirm_Order() {
    ...
    cartList.forEach(v => {
      v.haKuCun == false && v.y_isCheck === true
        ? (canSubmit_hasKuCun = false)
        : "";
    });
    ...
    util.promiseRequest(api.confirm_order, paramObj).then(res => {
  ...
  onPullDownRefresh: function() {
    ...
    async function showData() {
      ...
      // 多个地址显示右箭头
      var myshippingAddressLength = wx.Storage.getItem(
        "myshippingAddressLength"
      );
      if (
        myshippingAddressLength === "undefined" ||
